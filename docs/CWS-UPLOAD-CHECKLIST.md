...
**Pack:** `dist/cannabis-sage-1.3.1.zip`
...
| Pack zip + secret scan | PASS — `dist/cannabis-sage-1.3.1.zip`, manifest present, no `sk_`/`whsec_` |
...
| 6 | **Assay** | Point `extension/data/config.json` (`apiBaseUrl`, `upgradeUrl`, `accountUrl`) at prod; add prod origin to `manifest.json` `host_permissions`; re-pack zip | ☐ |
| 7 | **Shai** | Upload **`dist/cannabis-sage-1.3.1.zip`** (or re-packed prod zip) to Chrome Web Store | ☐ |
