...
Version: **extension 1.3.2** · Pack: `./scripts/pack-extension.sh` → `dist/cannabis-sage-1.3.2.zip`
...
6. **Point extension at prod API** — ✅ done in **1.3.2**: `extension/data/config.json` → `https://cannabissage.vercel.app`; manifest includes that origin (+ localhost for unpacked local/dev). Override local API via `csi_api_base` storage or temporary config edit.
