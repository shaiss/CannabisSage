...
2. **Stack flip-in (Neon + Checkout)** — durable licenses on Neon via `DATABASE_URL`; promo **$9/yr or $4/mo** via `STRIPE_PRICE_ID_PROMO` + `STRIPE_PRICE_ID_PROMO_MONTHLY` + `LAUNCH_DATE`; regular **$59/yr or $9/mo** via `STRIPE_PRICE_ID_REGULAR_ANNUAL` + `STRIPE_PRICE_ID_REGULAR_MONTHLY`. Schema: `web/db/migrations/001_licenses.sql`. CRUD stays in `web/lib/licenses.ts`.
...
5. **Set `LAUNCH_DATE`** to real public/CWS day (promo = +30 days). Regular tier **$59/yr or $9/mo** (`STRIPE_PRICE_ID_REGULAR_ANNUAL`, `STRIPE_PRICE_ID_REGULAR_MONTHLY`).
